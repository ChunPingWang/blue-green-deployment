...
flowchart TB
    ...
    subgraph K8s["Kubernetes 叢集"]
        subgraph Ingress["ingress-nginx 命名空間"]
            IC["Ingress Controller<br/>(入口)"]
        end

        subgraph GW["gateway 命名空間"]
            Gateway["Spring Cloud Gateway<br/>(路由決策)"]
        end

        subgraph Blue["blue 命名空間"]
            BlueService["Blue Service<br/>藍色版本"]
        end

        subgraph Green["green 命名空間"]
            GreenService["Green Service<br/>綠色版本"]
        end
    end

    User --> IC
    IC --> Gateway
    Gateway -->|"權重設定"| BlueService
    Gateway -->|"權重設定"| GreenService
